...
function generateFilePath({ mainPath, defaultMainFolder, layers, componentName }) {
    return layers.map(layer => {
        ...
        const filename = `${componentName}${Util.upperCaseFistLetter(layer)}.js`
...
describe('#Integration - Files - Files Structure', () => {
    ...
    test('Service should hava the same signature of repository and call all its methods', async () => {
        const myConfig = {
            ...config,
    ...
    test('Factory instance should math layers', async () => {
        const myConfig = {
            ...config
        }
